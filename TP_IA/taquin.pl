...
delete(N,X,[Y|L], [Y|R]) :-
	...
	*/
	...
	coordonnees([L,C], Mat, Elt) :- nth1(L,Mat,Ligne_result),nth1(C,Ligne_result,Elt).


   malplace(P,U,F) :- coordonnees([L_f,C_f],F,P),coordonnees([L_f,C_f],U,P_bis),P\=P_bis.
...
heuristique(U,H) :-
   ...
   % même piece.
    ...
    heuristique1(U, H) :- final_state(F),findall(Piece,   (malplace(Piece,U,F),Piece\=vide),     Liste),length(Liste,H).
   ...
   % entre sa position courante et sa positon dans l'etat final
        ...
        calculdistance(U,F,Elt,Result) :-
                coordonnees([L_a,C_a],U,Elt),
                coordonnees([L_f,C_f],F,Elt),
                RL_na is L_f - L_a,
                RC_na is C_f - C_a,
                RL is abs(RL_na),
                RC is abs(RC_na),
    ...
    heuristique2(U, H) :- final_state(F),findall(Result,
        (calculdistance(U,F,Elt,Result),Elt\=vide)    ,
        List_distance),sumlist(List_distance,H).
